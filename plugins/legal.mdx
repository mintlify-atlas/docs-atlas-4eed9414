...
<CardGroup cols={2}>
  ...
  <Card title="Compliance Check" icon="shield-check">
    Run compliance reviews on proposed actions, features, or business initiatives
  </Card>
  <Card title="Signature Request" icon="signature">
    Prepare and route documents for electronic signature
  </Card>
...
Evaluates applicable regulations (GDPR, CCPA, HIPAA, etc.), identifies requirements, assesses risks, and recommends actions. Output includes assessment summary, applicable regulations, requirements checklist, risk areas, and approvals needed.

### /signature-request

Prepare and route a document for electronic signature.

```bash
/signature-request [document or contract to send]
```

Verifies document completeness, configures signing order and recipients, performs pre-signature checklist, and routes for e-signature (if connected) or generates signing instructions.
